...
function grabLocalStorageRegistration() {
    ...
    var data = JSON.parse(getRegistrations);
    return data;
}
    ...
    signInFormSubmission.addEventListener("submit", function (e) {
        ...
        if (username.value !== "" && password.value !== "") {
            ...
            if (submitUsername.submit === submitPassword.submit) {
                localStorage.setItem("SignIn", JSON.stringify(submitUsername.submitArray.signIn));
...
function SignUpSubmission() {
    ...
    signUpFormSubmission.addEventListener("submit", function (e) {
        ...
        e.preventDefault();
        var getRegistrations = grabLocalStorageRegistration();
        var inputs = target.querySelectorAll("select, input ,textarea");
        for (var i = 0; i < inputs.length; i++) {
            var submitForm = inputValidation(inputs[i]);
            if (submitForm) {
                validation = true;
                // Object.assign(registerObject, { [inputs[i].name]: inputs[i].value });
                registerObject = __assign(__assign({}, registerObject), (_a = {}, _a[inputs[i].name] = inputs[i].value, _a));
            }
            else {
                validation = false;
                break;
        ...
        if (validation) {
            ...
            // });
            registerObject = __assign(__assign({}, registerObject), { id: generateId(), password: generatePassword() });
