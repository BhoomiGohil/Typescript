...
function checkLocalStorageSignin() {
  if (grabLocalStorageSignIn === "[]" || grabLocalStorageSignIn === null) {
    ...
    document.querySelector("#DBpassword").innerHTML = "".concat(
      data.password,
      " "
    );
  }
...
function inputValidation(input) {
  if (
    ...
    errorMessage(
      input,
      "Enter ".concat(
        input.name.charAt(0).toUpperCase() + input.name.substring(1)
      )
    );
    submit = false;
  } else if (input.value) {
...
function flights(from, to) {
  ...
  flightFound.innerHTML = "Flight results: ".concat(
    array.length,
    " flights found"
  );
}
  ...
  signUpFormSubmission.addEventListener("submit", function (e) {
    ...
    for (var i = 0; i < inputs.length; i++) {
      ...
      if (submitForm) {
        ...
        Object.assign(
          registerObject,
          ((_a = {}), (_a[inputs[i].name] = inputs[i].value), _a)
        );
      } else {
        validation = false;
        break;
